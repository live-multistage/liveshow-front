...
import type { ArtistFormValues } from './ArtistProfileForm';
import styles from './ArtistSlugField.module.scss';
...
export function ArtistSlugField({ control, excludeId, initialSlug }: Props) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      try {
        const { available } = await artistService.checkSlug(slug, excludeId);
  ...
  return (
    <div className={styles.field}>
      <label className={styles.label}>{t('dashboard.profile.slug')}</label>
